(function() {
  ...
  const CONFIG = {
    // IMPORTANT: Replace with your Google Apps Script URL for lead collection
    // Get your GAS URL from: https://script.google.com/
  ...
  const Tracker = {
    ...
    setupExitIntentPopup() {
      ...
      window.showFloorPlanPopup = () => {
        const popup = document.getElementById('floorplanPopup');
        if (!popup) return;
        if (popup.classList.contains('active')) return;
        popup.classList.add('active');
        this.trackBehavior('floorplan_popup_open', 'Image Click');
      };
      ...
      const floorplanPopupClose = document.getElementById('floorplanPopupClose');
      const floorplanForm = document.getElementById('floorplanForm');

      floorplanPopupClose?.addEventListener('click', () => {
        floorplanPopup?.classList.remove('active');
      });
      floorplanPopup?.addEventListener('click', (e) => { if (e.target === floorplanPopup) floorplanPopup.classList.remove('active'); });

      if (floorplanForm) {
        floorplanForm.addEventListener('submit', (e) => {
          e.preventDefault();
          this.validateAndSubmit(floorplanForm);
        });
      }
    ...
    validateAndSubmit(form) {
      ...
      this.trackLeadSubmission(data).then(success => {
        ...
        if (form.dataset.redirect === 'whatsapp' && success !== false) {
          // Old behavior - redirect to WhatsApp (kept for compatibility)
        } else if (form.dataset.redirect === 'thankyou' && success !== false) {
    ...
    initCarousels() {
      this.initAmenitiesCarousel();
      this.initFloorplanCarousel();
    },
    ...
    initAmenitiesCarousel() {
      ...
      const blocks = section.querySelectorAll('.amenity-block');
      if (blocks.length <= 1) return;

      const wrapper = document.createElement('div');
      wrapper.className = 'amenities-carousel';

      const slider = document.createElement('div');
      slider.className = 'amenity-slider';

      blocks.forEach(block => slider.appendChild(block));
      ...
      for (let i = 0; i < blocks.length; i++) {
        ...
        dot.addEventListener('click', () => this.goToSlide(slider, i, blocks.length));
        nav.appendChild(dot);
      }
      ...
      this.amenitiesSlider = { el: slider, index: 0, total: blocks.length };
    },

    initFloorplanCarousel() {
      const section = document.querySelector('.floorplan-section');
      if (!section) return;

      const grid = section.querySelector('.floorplan-grid-simple');
      const items = grid?.querySelectorAll('.floorplan-item');
      if (!items || items.length <= 1) return;

      const wrapper = document.createElement('div');
      wrapper.className = 'floorplan-carousel';
      ...
      nav.innerHTML = '<button class="slider-arrow" aria-label="Previous"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg></button><button class="slider-arrow" aria-label="Next"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg></button>';
      wrapper.appendChild(nav);

      const arrows = nav.querySelectorAll('.slider-arrow');
      arrows[0].addEventListener('click', () => this.prevFloorplanSlide(slider, items.length));
      arrows[1].addEventListener('click', () => this.nextFloorplanSlide(slider, items.length));

      grid?.replaceWith(wrapper);

      this.floorplanSlider = { el: slider, index: 0, total: items.length };
    },
    ...
    goToSlide(slider, index, total) {
      ...
      const moveAmount = item.getBoundingClientRect().width + gap;
      
      slider.style.transform = `translateX(-${index * moveAmount}px)`;
      const nav = slider.parentElement.querySelector('.slider-nav');
      if (nav) {
    ...
    nextFloorplanSlide(slider, total) {
      if (!slider || !slider.children.length) return;
      const item = slider.children[0];
      const gap = parseFloat(window.getComputedStyle(slider).gap) || 0;
      const moveAmount = item.getBoundingClientRect().width + gap;
      ...
      this.floorplanSlider.index = Math.min(maxIndex, this.floorplanSlider.index + 1);
      slider.style.transform = `translateX(-${this.floorplanSlider.index * moveAmount}px)`;
    },

    prevFloorplanSlide(slider, total) {
      if (!slider || !slider.children.length) return;
      const item = slider.children[0];
      const gap = parseFloat(window.getComputedStyle(slider).gap) || 0;
      const moveAmount = item.getBoundingClientRect().width + gap;
      
      this.floorplanSlider.index = Math.max(0, this.floorplanSlider.index - 1);
      slider.style.transform = `translateX(-${this.floorplanSlider.index * moveAmount}px)`;
    },
  ...
  (function() {
    ...
    // Form Validation
    const formInputs = document.querySelectorAll('.lead-form input[required], .lead-form select[required]');
    ...
    // Draggable Floating WhatsApp
    const fab = document.getElementById('floatingWhatsapp');
    if (fab) {
      ...
      const dragActive = (e) => {
        ...
        if (Math.abs(dx) > 5 || Math.abs(dy) > 5) {
            ...
            const rectWidth = fab.offsetWidth;
            const rectHeight = fab.offsetHeight;
            newLeft = Math.max(0, Math.min(window.innerWidth - rectWidth, newLeft));
            newTop = Math.max(0, Math.min(window.innerHeight - rectHeight, newTop));
      ...
      window.addEventListener('touchmove', dragActive, {passive: false});
  ...
  (function initVariableFontScroll() {
    const heroTitle = document.querySelector('.hero-title');
    if (!heroTitle) return;

    // Set an initial starting point for smooth interpolation
    heroTitle.style.fontVariationSettings = '"wght" 400';
    heroTitle.style.letterSpacing = '-0.05em';
    heroTitle.style.transition = 'font-variation-settings 0.1s ease-out, letter-spacing 0.1s ease-out';

    window.addEventListener('scroll', () => {
      // Calculate how far the user has scrolled down the first section
      const scrollY = window.scrollY;
      const maxScroll = window.innerHeight; // The range over which the bloom happens
      
      // Calculate progress from 0 to 1
      let progress = Math.min(Math.max(scrollY / maxScroll, 0), 1);
      
      // Interpolate weight: 400 to 900
      const currentWeight = 400 + (progress * 500);
      
      // Interpolate letter-spacing: -0.05em to 0.05em (emulating "width" expansion)
      const currentTracking = -0.05 + (progress * 0.1);

      heroTitle.style.fontVariationSettings = `"wght" ${currentWeight}`;
      heroTitle.style.letterSpacing = `${currentTracking}em`;
    }, { passive: true });
  })();
  // The Grab: Floating Invitation Modal
  (function initGrabModal() {
    if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;
    const modal = document.getElementById('grab-modal');
    const closeBtn = document.getElementById('close-grab-modal');
    const titleText = document.querySelector('.grab-variable-text');
    if (!modal || !closeBtn || !titleText) return;
    ...
    const triggerModal = () => {
      ...
      gsap.set(modal, { visibility: 'visible', y: 100 });
    ...
    // Trigger on scroll > 0.5
    ScrollTrigger.create({
      start: () => `top+=${document.documentElement.scrollHeight * 0.5} top`,
      onEnter: triggerModal,
      once: true
    });

    // Or trigger after 30 seconds
    setTimeout(triggerModal, 30000);
  ...
  // Liquid Magnetic Phone Reveal
  (function initLMPR() {
    const pills = document.querySelectorAll('.lmpr-pill');
    if (!pills.length) return;

    pills.forEach(pill => {
      const wrapper   = pill.closest('.lmpr-wrapper');
      const phoneRaw  = wrapper?.dataset.phone || '+918010473982';
      const display   = wrapper?.dataset.display || '+91 80104 73982';
      const numEl     = pill.querySelector('.lmpr-number');
      let revealed    = false;
      let magRaf      = null;
      ...
      const onMouseLeave = () => {
        pill.style.transform = '';
        pill.style.transition = 'transform 0.5s cubic-bezier(0.23, 1, 0.32, 1)';
      ...
      // ── Click / Tap: odometer reveal ──────────────────────────
      const rollOdometer = (target, finalText) => {
        ...
        const tick = () => {
          ...
          // For each char, show random digit until we settle
          const display = chars.map((ch, i) => {
            if (!digits.includes(ch)) return ch; // spaces, '+', ' '
            const charProgress = Math.max(0, progress - i * 0.04);
      ...
      pill.addEventListener('click', () => {
        ...
        revealed = true;
        pill.classList.add('lmpr-revealed');
        // Roll the number in
        setTimeout(() => rollOdometer(numEl, display), 200);
      });

      // Touch proximity on mobile (tilt toward touch)
      pill.addEventListener('touchmove', (e) => {
        ...
        const cx    = rect.left + rect.width / 2;
        const cy    = rect.top  + rect.height / 2;
        const dx    = Math.min(Math.max(touch.clientX - cx, -MAX_OFFSET), MAX_OFFSET);
        const dy    = Math.min(Math.max(touch.clientY - cy, -MAX_OFFSET), MAX_OFFSET);
        pill.style.transform = `translate(${dx * 0.4}px, ${dy * 0.4}px)`;
      }, { passive: true });
  ...
  // 3-Day Offer Countdown Timer (resets on a fixed epoch cycle)
  (function initCountdown() {
    ...
    const CYCLE_MS = 15 * 24 * 60 * 60 * 1000; // 15 days
    ...
    const tick = () => {
      ...
      hoursEl.textContent = pad(Math.floor(total / 3600) % 24);
      minsEl.textContent  = pad(Math.floor(total / 60) % 60);
